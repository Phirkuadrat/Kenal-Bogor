...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql = "INSERT INTO tempat_wisata (id_user, nama_tempat, deskripsi, gambar1, gambar2, gambar3, maps) VALUES ($user, '$namaTempat', '$deskripsi', '$gambar1', '$gambar2', '$gambar3', '$maps')";

    if ($conn->query($sql) === TRUE) {
        header("Location: ../tempatWisataPage.php?success=Tempat Wisata Berhasil Ditambahkan!");
        exit();
    } else {
        header("Location: ../tempatWisataPage.php?failed=Tempat Wisata Gagal Ditambahkan!");
        exit();
    }
    $conn->close();
}
