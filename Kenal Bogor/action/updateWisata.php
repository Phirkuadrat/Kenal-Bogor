...
if ($gambar1) {
    ...
    $query = "SELECT gambar1 FROM tempat_wisata WHERE id = ?";
...
if ($gambar2) {
    ...
    $query = "SELECT gambar2 FROM tempat_wisata WHERE id = ?";
    $stmt = $conn->prepare($query);
    $stmt->bind_param('i', $id);
    $stmt->execute();
    $result = $stmt->get_result();
...
if ($gambar3) {
    ...
    $query = "SELECT gambar3 FROM tempat_wisata WHERE id = ?";
    $stmt = $conn->prepare($query);
    $stmt->bind_param('i', $id);
    $stmt->execute();
    $result = $stmt->get_result();
...
$query = "UPDATE tempat_wisata SET nama_tempat=?, deskripsi=?, gambar1=?, gambar2=?, gambar3=?, maps=? WHERE id=?";
$stmt = $conn->prepare($query);
$stmt->bind_param('ssssssi', $namaTempat, $deskripsi, $gambar1, $gambar2, $gambar3, $maps, $id);

if ($stmt->execute()) {
    header("Location: ../tempatWisataPage.php?success=Tempat Wisata Berhasil Diubah!");
    exit();
} else {
    header("Location: ../tempatWisataPage.php?failed=Tempat Wisata Gagal Diubah!");
    exit();
}
